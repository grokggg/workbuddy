    ...
    for (const filename of filenamesSorted) {
        ...
        switch (file.type) {
            ...
            case 'file': {
                ...
                const duplicate = await filesystem.insertFile(filename, () => fs.createReadStream(filename), shouldUnpack, file, options);
    ...
    const handleFile = async function (stream) {
        ...
        switch (type) {
            case 'directory':
                ...
                break;
            case 'file': {
                const duplicate = await filesystem.insertFile(filename, stream.streamGenerator, stream.unpacked, { type: 'file', stat: stream.stat });
                files.push({
                    filename,
